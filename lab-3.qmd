...
### Row - Medals By Year {height="80%"}

```{r}

#wgm2018_full_new2 %>% filter(Education == '1') %>% count(Q12) %>%
#  mutate(prop = prop.table(n))



#wgm2018_full_new2 %>% filter(Education == '2') %>% count(Q12) %>%
#  mutate(prop = prop.table(n))



#wgm2018_full_new2 %>% filter(Education == '3') %>% count(Q12) %>%
#  mutate(prop = prop.table(n))
...
wgm2018_full_new2 %>% filter(Q12 %in% c(1,2,3,4)) %>%
  ggplot(aes(x = Education, fill = as.character(Q12))) + 
